...
void TestKeePass2Format::initTestCase()
{
	...
	bool hasError;
	QString errorString;
	m_xmlDb = readXml(QStringLiteral(KEEPASSX_TEST_DATA_DIR).append(QStringLiteral("/NewDatabase.xml")), true, hasError, errorString);
	if (hasError)
	{
	...
	key->addKey(QSharedPointer<PasswordKey>::create(QStringLiteral("test")));

	m_kdbxSourceDb = QSharedPointer<Database>::create();
	m_kdbxSourceDb->setKey(key);
	m_kdbxSourceDb->metadata()->setName(QStringLiteral("TESTDB"));
	Group *group = m_kdbxSourceDb->rootGroup();
	group->setUuid(QUuid::createUuid());
	group->setNotes(QStringLiteral("I'm a note!"));
	auto entry = new Entry();
	entry->setPassword(QString::fromUtf8("\xc3\xa4\xa3\xb6\xc3\xbc\xe9\x9b\xbb\xe7\xb4\x85"));
	entry->setUuid(QUuid::createUuid());
	entry->attributes()->set(QStringLiteral("test"), QStringLiteral("protectedTest"), true);
	QVERIFY(entry->attributes()->isProtected(QStringLiteral("test")));
	entry->attachments()->set(QStringLiteral("myattach.txt"), QByteArray("this is an attachment"));
	entry->attachments()->set(QStringLiteral("aaa.txt"), QByteArray("also an attachment"));
	entry->setGroup(group);
	auto groupNew = new Group();
	groupNew->setUuid(QUuid::createUuid());
	groupNew->setName(QStringLiteral("TESTGROUP"));
	groupNew->setNotes(QStringLiteral("I'm a sub group note!"));
...
void TestKeePass2Format::testXmlEntry1()
{
	...
	QCOMPARE(entry->attributes()->value(QStringLiteral("Notes")), QStringLiteral("Notes"));
	QVERIFY(!entry->attributes()->isProtected(QStringLiteral("Notes")));
	QVERIFY(attrs.removeOne(QStringLiteral("Notes")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Password")), QStringLiteral("Password"));
	QVERIFY(!entry->attributes()->isProtected(QStringLiteral("Password")));
	QVERIFY(attrs.removeOne(QStringLiteral("Password")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Title")), QStringLiteral("Sample Entry 1"));
	QVERIFY(!entry->attributes()->isProtected(QStringLiteral("Title")));
	QVERIFY(attrs.removeOne(QStringLiteral("Title")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("URL")), QStringLiteral(""));
	QVERIFY(entry->attributes()->isProtected(QStringLiteral("URL")));
	QVERIFY(attrs.removeOne(QStringLiteral("URL")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("UserName")), QStringLiteral("User Name"));
	QVERIFY(entry->attributes()->isProtected(QStringLiteral("UserName")));
	QVERIFY(attrs.removeOne(QStringLiteral("UserName")));
	QVERIFY(attrs.isEmpty());

	QCOMPARE(entry->title(), entry->attributes()->value(QStringLiteral("Title")));
	QCOMPARE(entry->url(), entry->attributes()->value(QStringLiteral("URL")));
	QCOMPARE(entry->username(), entry->attributes()->value(QStringLiteral("UserName")));
	QCOMPARE(entry->password(), entry->attributes()->value(QStringLiteral("Password")));
	QCOMPARE(entry->notes(), entry->attributes()->value(QStringLiteral("Notes")));
	...
	QCOMPARE(entry->historyItems().at(0)->attachments()->value(QStringLiteral("myattach.txt")), QByteArray("0123456789"));
...
void TestKeePass2Format::testXmlEntry2()
{
	...
	QList<QString> attrs = entry->attributes()->keys();
	QCOMPARE(entry->attributes()->value(QStringLiteral("CustomString")), QStringLiteral("isavalue"));
	QVERIFY(attrs.removeOne(QStringLiteral("CustomString")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Notes")), QString());
	QVERIFY(attrs.removeOne(QStringLiteral("Notes")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Password")), QStringLiteral("Jer60Hz8o9XHvxBGcRqT"));
	QVERIFY(attrs.removeOne(QStringLiteral("Password")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Protected String")), QStringLiteral("y")); // TODO: should have a protection attribute
	QVERIFY(attrs.removeOne(QStringLiteral("Protected String")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("Title")), QStringLiteral("Sample Entry 2"));
	QVERIFY(attrs.removeOne(QStringLiteral("Title")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("URL")), QStringLiteral("http://www.keepassx.org/"));
	QVERIFY(attrs.removeOne(QStringLiteral("URL")));
	QCOMPARE(entry->attributes()->value(QStringLiteral("UserName")), QStringLiteral("notDEFUSERNAME"));
	QVERIFY(attrs.removeOne(QStringLiteral("UserName")));
	QVERIFY(attrs.isEmpty());

	QCOMPARE(entry->attachments()->keys().size(), 1);
	QCOMPARE(QString::fromLatin1(entry->attachments()->value(QStringLiteral("myattach.txt"))), QStringLiteral("abcdefghijk"));
}
...
void TestKeePass2Format::testXmlInvalidXmlChars()
{
	...
	entry->attributes()->set(QStringLiteral("PlainInvalid"), strPlainInvalid);
	entry->attributes()->set(QStringLiteral("PlainValid"), strPlainValid);
	entry->attributes()->set(QStringLiteral("SingleHighSurrogate1"), strSingleHighSurrogate1);
	entry->attributes()->set(QStringLiteral("SingleHighSurrogate2"), strSingleHighSurrogate2);
	entry->attributes()->set(QStringLiteral("HighHighSurrogate"), strHighHighSurrogate);
	entry->attributes()->set(QStringLiteral("SingleLowSurrogate1"), strSingleLowSurrogate1);
	entry->attributes()->set(QStringLiteral("SingleLowSurrogate2"), strSingleLowSurrogate2);
	entry->attributes()->set(QStringLiteral("LowLowSurrogate"), strLowLowSurrogate);
	entry->attributes()->set(QStringLiteral("SurrogateValid1"), strSurrogateValid1);
	entry->attributes()->set(QStringLiteral("SurrogateValid2"), strSurrogateValid2);
	...
	QCOMPARE(attrRead->value(QStringLiteral("PlainInvalid")), QString());
	QCOMPARE(attrRead->value(QStringLiteral("PlainValid")), strPlainValid);
	QCOMPARE(attrRead->value(QStringLiteral("SingleHighSurrogate1")), QString());
	QCOMPARE(attrRead->value(QStringLiteral("SingleHighSurrogate2")), QStringLiteral("12"));
	QCOMPARE(attrRead->value(QStringLiteral("HighHighSurrogate")), QString());
	QCOMPARE(attrRead->value(QStringLiteral("SingleLowSurrogate1")), QString());
	QCOMPARE(attrRead->value(QStringLiteral("SingleLowSurrogate2")), QStringLiteral("12"));
	QCOMPARE(attrRead->value(QStringLiteral("LowLowSurrogate")), QString());
	QCOMPARE(attrRead->value(QStringLiteral("SurrogateValid1")), strSurrogateValid1);
	QCOMPARE(attrRead->value(QStringLiteral("SurrogateValid2")), strSurrogateValid2);
}
...
void TestKeePass2Format::testKdbxProtectedAttributes()
{
	...
	QCOMPARE(entry->attributes()->value(QStringLiteral("test")), QStringLiteral("protectedTest"));
	QCOMPARE(entry->attributes()->isProtected(QStringLiteral("test")), true);
}
...
void TestKeePass2Format::testKdbxAttachments()
{
	...
	QCOMPARE(entry->attachments()->value(QStringLiteral("myattach.txt")), QByteArray("this is an attachment"));
	QCOMPARE(entry->attachments()->value(QStringLiteral("aaa.txt")), QByteArray("also an attachment"));
}
...
void TestKeePass2Format::testKdbxDeviceFailure()
{
	...
	entry->attachments()->set(QStringLiteral("test"), attachment);
...
void TestKeePass2Format::testKdbxKeyChange_data()
{
	...
	auto passwordKey1 = QSharedPointer<PasswordKey>::create(QStringLiteral("abc"));
	auto passwordKey2 = QSharedPointer<PasswordKey>::create(QStringLiteral("def"));
	...
	QBuffer fileKeyBuffer2(&fileKeyBytes2);
...
void TestKeePass2Format::testDuplicateAttachments()
{
	...
	entry2->attachments()->set(QStringLiteral("b1"), attachment1);
	entry2->beginUpdate();
	entry2->attachments()->set(QStringLiteral("b2"), attachment1);
	entry2->endUpdate();
	entry2->beginUpdate();
	entry2->attachments()->set(QStringLiteral("b3"), attachment2);
	entry2->endUpdate();
	entry2->beginUpdate();
	entry2->attachments()->set(QStringLiteral("b4"), attachment2);
	entry2->endUpdate();

	auto entry3 = new Entry();
	entry3->setGroup(db->rootGroup());
	entry3->setUuid(QUuid::fromRfc4122("cccccccccccccccc"));
	entry3->attachments()->set(QStringLiteral("c1"), attachment2);
	entry3->attachments()->set(QStringLiteral("c2"), attachment2);
	entry3->attachments()->set(QStringLiteral("c3"), attachment3);
	...
	QCOMPARE(db->rootGroup()->entries()[0]->attachments()->value(QStringLiteral("a")), attachment1);

	QCOMPARE(db->rootGroup()->entries()[1]->attachments()->value(QStringLiteral("b1")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->attachments()->value(QStringLiteral("b2")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->attachments()->value(QStringLiteral("b3")), attachment2);
	QCOMPARE(db->rootGroup()->entries()[1]->attachments()->value(QStringLiteral("b4")), attachment2);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[0]->attachments()->value(QStringLiteral("b1")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[1]->attachments()->value(QStringLiteral("b1")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[1]->attachments()->value(QStringLiteral("b2")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[2]->attachments()->value(QStringLiteral("b1")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[2]->attachments()->value(QStringLiteral("b2")), attachment1);
	QCOMPARE(db->rootGroup()->entries()[1]->historyItems()[2]->attachments()->value(QStringLiteral("b3")), attachment2);

	QCOMPARE(db->rootGroup()->entries()[2]->attachments()->value(QStringLiteral("c1")), attachment2);
	QCOMPARE(db->rootGroup()->entries()[2]->attachments()->value(QStringLiteral("c2")), attachment2);
	QCOMPARE(db->rootGroup()->entries()[2]->attachments()->value(QStringLiteral("c3")), attachment3);
}
