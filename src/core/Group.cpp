...
inline bool Group::set(P &property, const V &value)
{
	if (property != value)
	{
		property = value;
...
void Group::setParent(Group *parent, int index, bool trackPrevious)
{
	...
	if (!moveWithinDatabase)
	{
		...
		if (m_db)
		{
			...
			if ((!iconUuid().isNull())
				&& parent->m_db
				&& m_db->metadata()->hasCustomIcon(iconUuid())
...
void Group::applyGroupIconOnCreateTo(Entry *entry)
{
	...
	if (((iconNumber() == Group::DefaultIconNumber)
			|| (iconNumber() == Group::OpenFolderIconNumber))
		&& iconUuid().isNull())
	{
		return;
	}
