...
bool Kdbx4Writer::serializeVariantMap(const QVariantMap &map, QByteArray &outputBytes)
{
	...
	for (const auto &k: keys)
	{
		...
		case QMetaType::Type::QByteArray:
			...
			break;
		default:
			qWarning() << tr("Unknown object type %1 in QVariantMap").arg(v.typeId());
			return false;
		}
		QByteArray typeBytes;
		typeBytes.append(static_cast<char>(fieldType));
		QByteArray nameBytes = k.toUtf8();
		QByteArray nameLenBytes = Endian::sizedIntToBytes<uint32_t>(nameBytes.size(), KeePass2::BYTEORDER);
		QByteArray dataLenBytes = Endian::sizedIntToBytes<uint32_t>(data.size(), KeePass2::BYTEORDER);
