...
void KdbxXmlReader::readDatabase(QIODevice *device, Database *db, KeePass2RandomStream *randomStream)
{
	...
	if (m_xml.readNextStartElement() && m_xml.name() == QStringLiteral("KeePassFile"))
	{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("KeePassFile"));
	...
	{
		if (m_xml.name() == QStringLiteral("Meta"))
		{
			parseMeta();
			continue;
		}

		if (m_xml.name() == QStringLiteral("Root"))
		{
			if (rootElementFound)
			{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Meta"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("Generator"))
		{
			m_meta->setGenerator(readString());
		}
		else if (m_xml.name() == QStringLiteral("HeaderHash"))
		{
			m_headerHash = readBinary();
		}
		else if (m_xml.name() == QStringLiteral("DatabaseName"))
		{
			m_meta->setName(readString());
		}
		else if (m_xml.name() == QStringLiteral("DatabaseNameChanged"))
		{
			m_meta->setNameChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("DatabaseDescription"))
		{
			m_meta->setDescription(readString());
		}
		else if (m_xml.name() == QStringLiteral("DatabaseDescriptionChanged"))
		{
			m_meta->setDescriptionChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("DefaultUserName"))
		{
			m_meta->setDefaultUserName(readString());
		}
		else if (m_xml.name() == QStringLiteral("DefaultUserNameChanged"))
		{
			m_meta->setDefaultUserNameChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("MaintenanceHistoryDays"))
		{
			m_meta->setMaintenanceHistoryDays(readNumber());
		}
		else if (m_xml.name() == QStringLiteral("Color"))
		{
			m_meta->setColor(readColor());
		}
		else if (m_xml.name() == QStringLiteral("MasterKeyChanged"))
		{
			m_meta->setDatabaseKeyChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("MasterKeyChangeRec"))
		{
			m_meta->setMasterKeyChangeRec(readNumber());
		}
		else if (m_xml.name() == QStringLiteral("MasterKeyChangeForce"))
		{
			m_meta->setMasterKeyChangeForce(readNumber());
		}
		else if (m_xml.name() == QStringLiteral("MemoryProtection"))
		{
			parseMemoryProtection();
		}
		else if (m_xml.name() == QStringLiteral("CustomIcons"))
		{
			parseCustomIcons();
		}
		else if (m_xml.name() == QStringLiteral("RecycleBinEnabled"))
		{
			m_meta->setRecycleBinEnabled(readBool());
		}
		else if (m_xml.name() == QStringLiteral("RecycleBinUUID"))
		{
			m_meta->setRecycleBin(getGroup(readUuid()));
		}
		else if (m_xml.name() == QStringLiteral("RecycleBinChanged"))
		{
			m_meta->setRecycleBinChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("EntryTemplatesGroup"))
		{
			m_meta->setEntryTemplatesGroup(getGroup(readUuid()));
		}
		else if (m_xml.name() == QStringLiteral("EntryTemplatesGroupChanged"))
		{
			m_meta->setEntryTemplatesGroupChanged(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("LastSelectedGroup"))
		{
			m_meta->setLastSelectedGroup(getGroup(readUuid()));
		}
		else if (m_xml.name() == QStringLiteral("LastTopVisibleGroup"))
		{
			m_meta->setLastTopVisibleGroup(getGroup(readUuid()));
		}
		else if (m_xml.name() == QStringLiteral("HistoryMaxItems"))
		{
		...
		else if (m_xml.name() == QStringLiteral("CustomData"))
		{
			parseCustomData(m_meta->customData());
		}
		else if (m_xml.name() == QStringLiteral("SettingsChanged"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("MemoryProtection"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("ProtectTitle"))
		{
			m_meta->setProtectTitle(readBool());
		}
		else if (m_xml.name() == QStringLiteral("ProtectUserName"))
		{
			m_meta->setProtectUsername(readBool());
		}
		else if (m_xml.name() == QStringLiteral("ProtectPassword"))
		{
			m_meta->setProtectPassword(readBool());
		}
		else if (m_xml.name() == QStringLiteral("ProtectURL"))
		{
			m_meta->setProtectUrl(readBool());
		}
		else if (m_xml.name() == QStringLiteral("ProtectNotes"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("CustomIcons"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("Icon"))
		{
			parseIcon();
		}
		else
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Icon"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("UUID"))
		{
			uuid = readUuid();
			uuidSet = !uuid.isNull();
		}
		else if (m_xml.name() == QStringLiteral("Data"))
		{
			iconData = readBinary();
			iconSet = true;
		}
		else if (m_xml.name() == QStringLiteral("Name"))
		{
			name = readString();
		}
		else if (m_xml.name() == QStringLiteral("LastModificationTime"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Binaries"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() != QStringLiteral("Binary"))
		{
			skipCurrentElement();
			continue;
		}
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("CustomData"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("Item"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Item"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("Key"))
		{
			key = readString();
			keySet = true;
		}
		else if (m_xml.name() == QStringLiteral("Value"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Root"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("Group"))
		{
		...
		else if (m_xml.name() == QStringLiteral("DeletedObjects"))
		{
			parseDeletedObjects();
		}
		else
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Group"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		...
		if (m_xml.name() == QStringLiteral("Notes"))
		{
			group->setNotes(readString());
			continue;
		}

		if (m_xml.name() == QStringLiteral("Tags"))
		{
			group->setTags(readString());
			continue;
		}

		if (m_xml.name() == QStringLiteral("IconID"))
		{
		...
		if (m_xml.name() == QStringLiteral("CustomIconUUID"))
		{
			QUuid uuid = readUuid();
			if (!uuid.isNull())
			{
		...
		if (m_xml.name() == QStringLiteral("Times"))
		{
			group->setTimeInfo(parseTimes());
			continue;
		}

		if (m_xml.name() == QStringLiteral("IsExpanded"))
		{
			group->setExpanded(readBool());
			continue;
		}

		if (m_xml.name() == QStringLiteral("EnableSearching"))
		{
			QString str = readString();

			if (str.compare(QStringLiteral("null"), Qt::CaseInsensitive) == 0)
			{
				group->setSearchingEnabled(Group::Inherit);
			}
			else if (str.compare(QStringLiteral("true"), Qt::CaseInsensitive) == 0)
			{
		...
		if (m_xml.name() == QStringLiteral("LastTopVisibleEntry"))
		{
		...
		if (m_xml.name() == QStringLiteral("Entry"))
		{
		...
		if (m_xml.name() == QStringLiteral("CustomData"))
		{
			parseCustomData(group->customData());
			continue;
		}

		if (m_xml.name() == QStringLiteral("PreviousParentGroup"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("DeletedObjects"));

	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("DeletedObject"))
		{
			parseDeletedObject();
		}
		else
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("DeletedObject"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		...
		if (m_xml.name() == QStringLiteral("DeletionTime"))
		{
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Entry"));
	...
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		...
		if (m_xml.name() == QStringLiteral("ForegroundColor"))
		{
			entry->setForegroundColor(readColor());
			continue;
		}

		if (m_xml.name() == QStringLiteral("BackgroundColor"))
		{
			entry->setBackgroundColor(readColor());
			continue;
		}

		if (m_xml.name() == QStringLiteral("OverrideURL"))
		{
		...
		if (m_xml.name() == QStringLiteral("String"))
		{
			parseEntryString(entry);
			continue;
		}

		if (m_xml.name() == QStringLiteral("QualityCheck"))
		{
		...
		if (m_xml.name() == QStringLiteral("History"))
		{
			if (history)
			{
		...
		if (m_xml.name() == QStringLiteral("CustomData"))
		{
			...
			{
				entry->setExcludeFromReports(entry->customData()->value(CustomData::ExcludeFromReportsLegacy) == TRUE_STR);
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("String"));
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Binary"));
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("History"));
...
{
	Q_ASSERT(m_xml.isStartElement() && m_xml.name() == QStringLiteral("Times"));

	TimeInfo timeInfo;
	while (!m_xml.hasError() && m_xml.readNextStartElement())
	{
		if (m_xml.name() == QStringLiteral("LastModificationTime"))
		{
			timeInfo.setLastModificationTime(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("CreationTime"))
		{
			timeInfo.setCreationTime(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("LastAccessTime"))
		{
			timeInfo.setLastAccessTime(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("ExpiryTime"))
		{
			timeInfo.setExpiryTime(readDateTime());
		}
		else if (m_xml.name() == QStringLiteral("Expires"))
		{
			timeInfo.setExpires(readBool());
		}
		else if (m_xml.name() == QStringLiteral("UsageCount"))
		{
			timeInfo.setUsageCount(readNumber());
		}
		else if (m_xml.name() == QStringLiteral("LocationChanged"))
		{
...
QString KdbxXmlReader::readColor()
{
	...
	if (colorStr.length() != 7 || colorStr[0] != QLatin1Char('#'))
	{
		if (m_strictMode)
		{
