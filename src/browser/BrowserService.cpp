                                             ...
                                             bool passkey)
{
	...
	for (const auto &group: rootGroup->groupsRecursive(true))
	{
		if (group->isRecycled() || group->resolveCustomDataTriState(BrowserService::OPTION_HIDE_ENTRY) == Group::Enable)
		{
			continue;
		}
...
BrowserService::Access BrowserService::checkAccess(const Entry *entry,
                                                   const QString &siteHost,
                                                   const QString &formHost,
                                                   const QString &realm)
{
...
bool BrowserService::handleURLWithWildcards(const QUrl &entryQUrl, const QString &siteUrl)
{
	...
	QUrl siteQUrl = siteUrl;
	if (!matchWithRegex(entryQUrl.host(), siteQUrl.host(), true) || !matchWithRegex(entryQUrl.path(), siteQUrl.path()))
	{
		return false;
	}
