...
public:
	 ...
	 */
	template <typename T>
	static QDBusObjectPath objectPathSafe(T *object)
	{
		if (object)
		{
	...
	template <typename T>
	static QDBusObjectPath objectPathSafe(QPointer<T> object)
	{
	 ...
	 */
	template <typename T>
	static QList<QDBusObjectPath> objectsToPath(QList<T *> objects)
	{
	 ...
	 */
	template <typename T>
	T* pathToObject(const QDBusObjectPath &path) const
	{
	 ...
	 */
	template <typename T>
	QList<T*> pathsToObject(const QList<QDBusObjectPath> &paths) const
	{
		QList<T*> res;
...
private:
	...
	bool objectPropertyGetAll(
		const DBusClientPtr &client,
		DBusObject *obj,
		const QString &interface,
		const QDBusMessage &msg);

	static bool deliverMethod(
		const DBusClientPtr &client,
		DBusObject *obj,
